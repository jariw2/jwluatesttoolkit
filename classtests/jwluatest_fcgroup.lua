function FCGroup_PropertyTests(group)
    ...
    local curr_barline_style = nil
    if group.BarlineStyle then
        curr_barline_style = group.BarlineStyle
        group.BarlineStyle = finale.BARLINE_SHAPE -- this should not be used in modern Finale, but this is a test
    end
    NumberPropertyTest(group, "FCGroup", "BarlineShapeID", {1, 10, 100}) -- not sure if this is still usable in current Finale
    if curr_barline_style ~= nil then
        group.BarlineStyle = curr_barline_style
    end
